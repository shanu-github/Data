...
library(doParallel)
library(foreach)
source("C:/Work/Forecasting/Forecast_Paper/Required_Functions_year.R")

m3_data<- read.csv("C:/Work/Forecasting/Forecast_Paper/Year_data.csv", stringsAsFactors = FALSE)
m3_data<- melt(m3_data, id=c("Series", "N", "NF","Category","Starting.Year")) 
...
m3_data$start_date<- ymd(paste(m3_data$Starting.Year,'01','01',sep="-"))
...
for(series in unique_series)
{
  ...
  date_genearte<- seq(from=series_data$start_date[1],length= nrow(series_data),by="year")
  series_train_data<- data.frame(Date= date_genearte,value=series_data$value)
  test_end= nrow(series_train_data)-6
  ...
  for(i in test_begin:test_end)
  {
    ...
    Forecast_To<-seq(from=last_month,length= Forecast_horizon+1,by="year")[2:( Forecast_horizon+1)]
    ...
    merge_forecast= rbind(ets_forecast,arima_forecast,theta_forecast,snaive_forecast,naive_forecast)
...
write.csv( all_series_forecast1,"C:/Work/Forecasting/Forecast_Paper/forecast_result_year.csv", row.names = FALSE)
